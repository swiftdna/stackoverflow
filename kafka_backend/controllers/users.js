...
const getUserDetails = async (req, callback ) => {
  ...
  const userTagQuestions = async (req, callback ) => {
    try {
      ...
      const users = await Question.find({$and : [{ author: req.user.id},{tags: { $all: req.body.tag }}]}).lean()
      return callback(null, {
        data : users
    }); 
  }
  ...
  const getUserStats = async (req, callback ) => {
	try {
      ...
      const reputationcount= await User.find({_id:mongoose.Types.ObjectId(req.user.id)},{Reputation:1});
  ...
  const topUserTags = async (req, callback ) => {
    ...
    try {
          ...
          const user = await User.findById(
            { "_id" : mongoose.Types.ObjectId(req.user.id) },{"tags_score":1,"tags_post_count":1}).lean();
      ...
      try {
           if (req.query.tab === 'question' || req.query.tab === 'all')
           {
            question = await Question.find(
              { "author" : mongoose.Types.ObjectId(req.user.id)}).lean().limit(10);
        ...
        if (req.query.sortType === "score")
        {
          consolidated.sort( (a, b) => b.votesscore - a.votesscore);
            ...
            return callback(null,{ data : consolidated,
        ...
        if (req.query.tab === 'reputation')
        {
          try {
            const question = await Question.find(
              { "author" : mongoose.Types.ObjectId(req.user.id)}).lean();
              //console.log(question.length);
              question.map(ques => {
                ques.type="question",
               ques.reputationtime=ques.created;
                ques.reputation =0;
                count = 0;
               // console.log(ques.votes);
                ques.votes && ques.votes.length && ques.votes.map(vote=> {
                  if(vote.vote === 1)
                  {
                        ...
                        },
                 {
                   ...
                   answer.map(answ => {
                     answ.answers.map(answdt=> {
                      ...
                      answdt.votes && answdt.votes.length && answdt.votes.map(vote=> {
                        if(vote.vote === 1)
                        {
